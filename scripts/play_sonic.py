"""
Interactive SONIC game loop with real-time keyboard control.
Uses tinyworlds inference pipeline for single-frame prediction.

Controls:
  WASD + Space: mapped actions (0-3 for 4 actions)
...
class SonicGameState:
    """Manages game state and inference."""
    ...
    def render(self):
        ...
        cv2.putText(canvas, "ESC=Quit  WASD/Sp=Act  0-9=Direct", 
...
def main():
    ...
    # Action mapping (for 4-action setup)
    action_map = {
        ord('w'): 0,  # up
        ord('a'): 1,  # left
        ord('s'): 2,  # down
    ...
    # Game loop
    window_name = "SONIC - Interactive Inference"
