...
OffloadPolicyType = CPUOffloadPolicy if CPUOffloadPolicy is not None else Any
...
class FSDPMixedPrecisionConfig:
    ...
    def _resolve_dtype(self, value: Union[str, torch.dtype]) -> torch.dtype:
    ...
    def to_policy(self) -> MixedPrecisionPolicy:
        if MixedPrecisionPolicy is None:
            raise RuntimeError("FSDP mixed precision is unavailable in this PyTorch build.")
...
class DistributedConfig:
    ...
    fsdp_mixed_precision: Optional[FSDPMixedPrecisionConfig] = field(default_factory=FSDPMixedPrecisionConfig)
    offload_policy: Optional[OffloadPolicyType] = None

    def __post_init__(self) -> None:
        if self.use_ddp and self.use_fsdp:
            raise ValueError("DistributedConfig cannot enable both DDP and FSDP; choose only one.")

    def get_mixed_precision_policy(self) -> Optional[MixedPrecisionPolicy]:
        if MixedPrecisionPolicy is None:
            return None
